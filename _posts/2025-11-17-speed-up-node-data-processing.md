...
Here we process each file one-by-one and write its result to the disk. We continue only after waiting the write operation to complete. This works and it's simple, but does not have much parallel processing going on. Let's improve it by running the write operation in the background:
...
Here we do not wait for file write to complete, but schedule it and immediately continue processing the next file. Assuming we have a large number of files to write, running this would eventually cause many files to be sent to **libuv** to be handled at once — potentially risking high memory peak. While libuv has some limits of how many write operations it does in parallel by default, the remaining operations are put into queue, which consumes a lot of memory. We need to act to prevent that.

What we actually want to do is to run multiple file write operations in parallel and in the background, but avoid running _too many_ and cause high memory peaks. `p-limit` solves this by allowing us to specify the maximum number of concurrently run promises, ensuring that only a set number of tasks run at any given time while the rest wait in a queue.

The right amount concurrent operations depends on the use case and the capability of the underlying hardware. I would suggest beginning with a value of 4-6 and finding the sweet spot manually by testing with real-world data.
...
Node.js has `worker_threads` module, which enables true parallelism by running code inside Workers. This practically frees us from the single-threaded limitation of Node. Hooray, data processing problems solved! But there is a catch: running code in a worker is _kind of_ like running another Node inside Node. Each worker has it's own execution context, call stack, heap memory and event loop. This makes worker threads completely separated, but also requires some effort to setup.
...
function runWorker(params: WorkerParams) {
...
async function main() {
  ...
  const workerPromise = runWorker(params);
...
This example shows how to offload computation from main thread to a dedicated worker while allowing main thread to be responsive and continue handling other tasks without being blocked. While this method works, it requires us to manually initialise a new Worker every time we want to process something in parallel. Also, if we want to run multiple different processes in parallel, the worker must be tuned to handle different tasks and return different results. This process can be simplified by using a library called [workerpool](https://github.com/josdejong/workerpool). 

### workerpool

Here is a simplified example of what **workerpool** looks like:
...
// This worker is able to execute these operations:
workerpool.worker({
  fibonacci: fibonacci,
});
```
...
**workerpool** takes the imlementation of creating and calling workers to a higher abtraction level, simplifying code and also making it more efficient since **workerpool** keeps a pool of workers "warm", ready to be used for offloading multiple long-running computations in the background. No need to manually create them every time.

In heavily parallelized applications, we have to be careful of not offloading too many operations to workers at once and thus introducing potential memory peaks. If more tasks are to be offloaded to workers than there are free workers available, we could use a manually defined pending queue to force the main thread to wait before a new worker would be initialized.
